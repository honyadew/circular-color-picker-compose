 ...
 */
@Composable
internal fun Magnifier(position: Offset, color: HsvColor, diameter: Dp, borderColor: Color) {
    ...
    Column(offset.size(width = diameter, height = diameter)) {
        MagnifierSelectionCircle(Modifier.size(diameter), color, borderColor)
    }
}
 ...
 */
@Composable
private fun MagnifierSelectionCircle(modifier: Modifier, color: HsvColor, borderColor: Color) {
    Surface(
        modifier = modifier,
        shape = CircleShape,
